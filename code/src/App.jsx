...
import { parseGraphHC, runHillClimbing } from './utils/hc';
...
function App() {
  ...
  // map lỗi BNB
  const mapBNBErr = (err) => {
    if (err?.name === 'BNBParseError' && err.code) return t(`errors.${err.code}`, err.ctx || {});
    return t('errors.BNB_ERR_GENERIC');
  };
  // map lỗi HC
  const mapHCErr = (err) => {
    if (err?.name === 'HCParseError' && err.code) return t(`errors.${err.code}`, err.ctx || {});
    return t('errors.HC_ERR_GENERIC');
  };
...
`;
        ...
        const { graph, h, start, goal } = parseGraphHC(fileContent);
        setHcData({ graph, h, start, goal });
      }
    } catch (e) {
      setError(mode === 'BNB' ? mapBNBErr(e) : mode === 'HC' ? mapHCErr(e) : e.message);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const runBNBClicked = async () => {
    ...
    } catch (e) { setError(mapBNBErr(e)); } finally { setIsLoading(false); }
  };

  const runHCClicked = async () => {
    if (!hcData) return;
    setIsLoading(true); setError(null);
    try {
      await new Promise((r) => setTimeout(r, 200));
      const { graph, h, start, goal } = hcData;
      setHcResults(runHillClimbing(graph, h, start, goal));
    } catch (e) { setError(mapHCErr(e)); } finally { setIsLoading(false); }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
            ...
            <div className="mt-6 border-b">
              <nav className="-mb-px flex space-x-6" aria-label="Tabs">
                ...
                <button
                  ...
                  className={`${tabBase} ${mode === 'HC' ? tabActive : tabInactive}`}
                >
                  Hill Climbing
                </button>
              </nav>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Upload + expected format preview paths */}
        <FileUpload
          mode={mode}
        ...
        {mode === 'HC' && hcData && (
          <div className="bg-white rounded-lg shadow-md p-6 mb-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-semibold text-gray-800">Hill Climbing</h2>
              <button onClick={resetAll} className="text-sm text-gray-500 hover:text-gray-700 underline">
                {t('graphInfo.reset')}
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
              <div className="bg-blue-50 p-3 rounded-lg">
                <span className="text-sm font-medium text-blue-700">{t('graphInfo.startNode')}</span>
                <span className="ml-2 text-blue-900 font-semibold">{hcData.start}</span>
              </div>
              <div className="bg-green-50 p-3 rounded-lg">
                <span className="text-sm font-medium text-green-700">{t('graphInfo.goalNode')}</span>
                <span className="ml-2 text-green-900 font-semibold">{hcData.goal}</span>
              </div>
              <div className="bg-gray-50 p-3 rounded-lg">
                <span className="text-sm font-medium text-gray-700">{t('graphInfo.totalNodes')}</span>
                <span className="ml-2 text-gray-900 font-semibold">
                  {Object.keys(hcData.graph).length}
                </span>
              </div>
            </div>
            ...
            <div className="mb-4">
              ...
              <div className="bg-gray-50 p-3 rounded-lg">
                <pre className="text-xs text-gray-600 font-mono">
                  {Object.entries(hcData.graph)
                    .map(([u, vs]) => `${u}: ${vs.join(', ') || t('graphInfo.noNeighbors')} | h=${hcData.h[u]}`)
                    .join('\n')}
                </pre>
              </div>
            </div>
            ...
            >
              {isLoading ? t('algo.hcRunning') : t('algo.hc')}
            </button>
          </div>
        )}
